...
def extract_inbound_message(payload: Dict[str, Any]) -> Optional[Dict[str, str]]:
    ...
    entry[0].changes[0].value.messages[0]. Also reads value.metadata.phone_number_id
    — the number the message was sent TO — which is the routing key the multi-tenant
    client registry uses to resolve which client this message belongs to."""
    try:
